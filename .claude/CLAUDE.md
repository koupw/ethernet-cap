...
| 模块 | 职责 |
|------|------|
| `main.c` | CLI 解析、初始化、`_kbhit`+`PeekNamedPipe` 双重 stdin 检测（兼容终端/GUI管道）、每 10ms 检查总量上限 |
...
| `gui/launcher.py` | Python tkinter GUI，`subprocess.Popen` 启动 CLI（`CREATE_NO_WINDOW` 无终端弹窗），管道捕获输出，自动参数持久化 |
...
- **内核 UDP 接收缓冲**：`SO_RCVBUF` 设为 64MB，减少千兆线速下内核丢包
